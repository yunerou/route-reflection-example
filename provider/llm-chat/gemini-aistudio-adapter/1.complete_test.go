...
func newTestClient(t *testing.T, handler http.HandlerFunc) llmchat.Adapter {
	t.Helper()
...
func TestComplete_Success(t *testing.T) {
	client := newTestClient(t, okHandler(`{"doc_type":"api-docs"}`))
...
func TestComplete_SetsAPIKeyHeader(t *testing.T) {
	var gotKey string
	...
	if _, aerr := client.Complete(context.Background(), llmchat.CompleteRequest{User: "u"}); aerr != nil {
		t.Fatalf("unexpected error: %v", aerr)
	}
	if gotKey != "test-key" {
		t.Fatalf("expected x-goog-api-key=test-key, got %q", gotKey)
	}
}
...
func TestComplete_URLPath(t *testing.T) {
	...
	if !strings.HasSuffix(gotPath, "/models/gemini-1.5-flash:generateContent") {
...
func TestComplete_OmitsSystemWhenEmpty(t *testing.T) {
	...
	client := newTestClient(t, handler)
	_, _ = client.Complete(context.Background(), llmchat.CompleteRequest{User: "u"})
...
func TestComplete_SendsUserContent(t *testing.T) {
	...
	client := newTestClient(t, handler)
	_, _ = client.Complete(context.Background(), llmchat.CompleteRequest{User: "hello"})
...
func TestComplete_OmitsResponseMimeTypeWhenNotJSON(t *testing.T) {
	var body map[string]any
	handler := func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(b, &body)
	...
	_, _ = client.Complete(context.Background(), llmchat.CompleteRequest{User: "u"})
	gc, _ := body["generationConfig"].(map[string]any)
	if _, exists := gc["responseMimeType"]; exists {
		t.Fatalf("responseMimeType should be omitted when ResponseFormat is nil")
	}
}
...
func TestComplete_SendsTempAndMaxTokens(t *testing.T) {
	...
	client := newTestClient(t, handler)
	_, _ = client.Complete(context.Background(), llmchat.CompleteRequest{
		User:        "u",
...
func TestComplete_Non200ReturnsError(t *testing.T) {
	...
	client := newTestClient(t, handler)
	_, aerr := client.Complete(context.Background(), llmchat.CompleteRequest{User: "u"})
	if aerr == nil {
