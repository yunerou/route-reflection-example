...
func buildRequest(in llmchat.CompleteRequest) genRequest {
	body := genRequest{
		Contents: []genContent{
...
func decodeResponse(ctx context.Context, body []byte, label string) (*llmchat.CompleteResponse, aerror.AError) {
	var decoded genResponse
	if err := json.Unmarshal(body, &decoded); err != nil {
		return nil, aerror.New(ctx, aerror.ErrUnexpectedNetwork, fmt.Errorf("%s: decode response: %w", label, err))
	}
...
func (c *client) Complete(ctx context.Context, in llmchat.CompleteRequest) (*llmchat.CompleteResponse, aerror.AError) {
	const label = "gemini-aistudio-chat"

	body := buildRequest(in)
	...
	return decodeResponse(ctx, respBody, label)
}
